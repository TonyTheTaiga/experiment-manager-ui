...
def train_epoch(model, device, train_loader, optimizer, criterion, epoch, tora, binary=True):
...
if __name__ == "__main__":
    ...
    for epoch in range(1, hyperparams["epochs"] + 1):
        ...
        if hyperparams["binary"]:
            ...
            train_loss = train_epoch(
                model, device, train_loader, optimizer, criterion, epoch, tora, binary=False
            )

        # Validate
        val_results = validate(
            model, device, val_loader, criterion, epoch, tora,
    ...
    final_results = validate(
        model, device, val_loader, criterion, hyperparams["epochs"], tora,
        binary=hyperparams["binary"], k=hyperparams["k"]
    )

...
class HyperparameterAdvisor:
    ...
    def __init__(
        ...
        if not self.api_key:
            print(
                "Warning: No API key provided. Set LLM_API_KEY environment variable or pass api_key."
            )
        if not self.llm_endpoint:
            print(
                "Warning: No LLM endpoint provided. Set LLM_ENDPOINT environment variable or pass llm_endpoint."
            )
    ...
    def analyze_training_trends(
        ...
        if lr_values:
            ...
            analysis["lr_decay_factor"] = (
                analysis["final_lr"] / analysis["initial_lr"]
                if analysis["initial_lr"] > 0
                else 0
            )
        return analysis

    def generate_prompt(
        self, experiment_data: Dict[str, Any], analysis: Dict[str, Any]
    ) -> str:
        """
...
"""
    ...
    def get_llm_suggestions(self, prompt: str) -> Dict[str, Any]:
        ...
        if not self.api_key or not self.llm_endpoint:
            raise ValueError(
                "API key and endpoint must be provided for LLM suggestions"
            )
        headers = {
        ...
        data = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You are an expert machine learning advisor specializing in hyperparameter optimization.",
                },
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter Advisor - Get ML model improvement suggestions"
    )
    parser.add_argument(
        "--experiment",
        "-e",

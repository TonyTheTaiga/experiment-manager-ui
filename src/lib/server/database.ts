...
import type { Experiment, ExperimentAndMetrics, HyperParam, Metric } from '$lib/types';
...
class DatabaseClient {
  ...
  private static getInstance(): SupabaseClient<Database> {
    if (!DatabaseClient.instance) {
      DatabaseClient.instance = createClient<Database>(
        PUBLIC_SUPABASE_URL,
  ...
  static async getExperimentAndMetrics(id: string): Promise<ExperimentAndMetrics> {
    const { data, error } = await DatabaseClient.getInstance()
      .from('experiment')
      .select('*, metric (*)')
      .eq('id', id)
  ...
  static async deleteExperiment(id: string): Promise<void> {
    const { error } = await DatabaseClient.getInstance().from('experiment').delete().eq('id', id);
  ...
  static async updateExperiment(id: string, update: { [key: string]: any }): Promise<void> {
    const { data, error } = await DatabaseClient.getInstance()
      .from('experiment')
      .update(update)
      .eq('id', id)
    ...
    if (error) {
      throw new Error(`Failed to update experiment with ID ${id}: ${error.message}`);
    }
  }
  ...
  static async createMetric(metric: Metric): Promise<void> {
    const { error } = await DatabaseClient.getInstance().from('metric').insert(metric);
  ...
  static async batchCreateMetric(metrics: Metric[]): Promise<void> {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        const { error } = await DatabaseClient.getInstance().from('metric').insert(metrics);

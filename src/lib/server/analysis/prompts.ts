 ...
 */
export function formatExperimentForLLM(data: ExperimentAndMetrics): string {
...
export function formatExperimentForHyperparameterAnalysis(data: ExperimentAndMetrics): string {
  const { experiment, metrics } = data;

  const experimentDescription = formatExperimentForLLM(data);
  
  // Create example JSON with actual hyperparameters from this experiment
  let exampleJson = '{\n';
  if (experiment.hyperparams && experiment.hyperparams.length > 0) {
    experiment.hyperparams.forEach((param, index) => {
      exampleJson += `  "${param.key}": "Suggestion to improve ${param.key} value"`;
      if (index < experiment.hyperparams!.length - 1) {
        exampleJson += ',\n';
      } else {
        exampleJson += '\n';
      }
    ...
    exampleJson += '  "example_param": "No hyperparameters found in this experiment"\n';
  }
  exampleJson += '}';

  let prompt = `
Based on the following experiment data:

${experimentDescription}

Create a JSON object where:
- Each key is one of the hyperparameters
- Each value is a string containing a suggestion on how to modify this hyperparameter to get better results
- Only focus on the most impactful changes

The JSON should follow this structure:
${exampleJson}

ONLY output the JSON dictionary, nothing else. No explanations, prefixes, or commentary.
`;

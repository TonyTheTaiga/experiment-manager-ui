<script lang="ts">
	...
	import { Plus, X, Tag as TagIcon, Settings, Blocks } from "lucide-svelte";
...
<form method="POST" action="?/create" class="flex flex-col gap-6">
	...
	<div class="space-y-3">
		...
		{#each hyperparams as hyperparam, i}
		...
		<button
			...
			onclick={() => (hyperparams = [...hyperparams, { key: "", value: "" }])}
		>
			<Plus size={12} />
			Add Parameter
		</button>
	...
	<!-- Experiment Refereces -->
	<div>
		<div class="flex items-center gap-2">
			<Blocks size={16} class="text-[var(--color-ctp-mauve)]" />
			<h3 class="text-lg font-semibold text-[var(--color-ctp-blue)]">
				References
			</h3>
		</div>

...
const chartCanvas: HTMLCanvasElement | null = $state(null);
const { experiment }: { experiment: Experiment } = $props();
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            caretPadding: 10,
            callbacks: {
              title: (tooltipItems) => `Step ${tooltipItems[0].label}`,
  ...
  try {
    ...
    if (chart_targets && chart_targets.length > 0) {
      ...
      const values = chart_targets.map((l) =>
        typeof l.value === 'number' ? l.value : Number.parseFloat(l.value) || 0
      );

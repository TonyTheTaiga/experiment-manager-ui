...
function formatHyperparameterAnalysis(
  paramsNames: string[],
  output: string,
): string | null {
  try {
...
export async function GET({ params: { slug } }: { params: { slug: string } }) {
  ...
  if (!experiment.experiment.hyperparams) {
    return new Response(
      JSON.stringify({ error: "No hyperparameters found in this experiment" }),
      { status: 400 },
    );
  }
  ...
  const formattedAnalysis = formatHyperparameterAnalysis(
    experiment.experiment.hyperparams.map((h) => h.key),
    analysis,
  );
  if (!formattedAnalysis) {
    return new Response(JSON.stringify({ error: "Invalid JSON output" }), {
      status: 400,
    });
  }
